        ...
        if entry ~= "." and entry ~= ".." then
            ...
            if mode == "directory" and entry:match("%.koplugin$") then
                ...
                if lfs.attributes(meta_file, "mode") == "file" then
                    ...
                    local ok, loaded_meta = pcall(dofile, meta_file)
                    if ok and loaded_meta and type(loaded_meta) == "table" then
                ...
                -- Add plugin even if _meta.lua is missing or failed to load (for install candidate matching)
                plugins[plugin_name] = {
                    name = plugin_name,
                    fullname = (has_meta and meta_data and meta_data.fullname) or plugin_name,
                    version = (has_meta and meta_data and meta_data.version) or "unknown",
                    description = (has_meta and meta_data and meta_data.description) or "",
                    path = plugin_path,
                    entry = entry,
                    meta = meta_data,
                }
...
function PluginManager.matchPluginToRepo(plugin_name, repo_name, installed_plugin)
    ...
    if installed_plugin and installed_plugin.entry then
        ...
        -- Strategy 5: Fuzzy match between entry name and repo name
        -- Normalize by removing non-alphanumeric characters and lowercasing.
        -- This helps match things like:
        --   entry: "menu_customizer.koplugin"
        --   repo:  "Koreader-Menu-customizer"
        local norm_entry = entry_base:lower():gsub("[^%w]", "")
        local norm_repo  = repo_base:lower():gsub("[^%w]", "")

        if norm_entry == norm_repo then
            return true
        end

        -- Allow repo name to have a prefix like "koreader-"
        if #norm_repo > #norm_entry and norm_repo:sub(-#norm_entry) == norm_entry then
            return true
        end

        if #norm_entry > #norm_repo and norm_entry:sub(-#norm_repo) == norm_repo then
            return true
        end
    end
...
-- Find ZIP asset in a GitHub release (supports optional repo_config.asset_pattern)
function PluginManager.findZipAsset(release_data, repo_config)
    ...
    for _, asset in ipairs(release_data.assets or {}) do
        if asset.name and asset.name:match(lua_pattern) then
            zip_asset = asset
            break
        end
    end

    return zip_asset, asset_pattern, lua_pattern
end

-- List install candidates from repositories (repos that are NOT installed)
-- Returns a list of "update-like" tables: { installed_plugin = {...}, repo_config = {...}, release = {...} }
function PluginManager.getInstallCandidates(plugin_repos, installed_plugins, httpGet, parseJSON, rateLimit)
    ...
    -- Also build mapping by entry (directory name) for direct lookup
    local installed_map = {}
    local installed_by_entry = {}
    for plugin_name, plugin_data in pairs(installed_plugins or {}) do
        installed_map[plugin_name] = plugin_data
        if plugin_data.entry then
            local entry_base = plugin_data.entry:gsub("%.koplugin$", "")
            installed_by_entry[entry_base] = plugin_data
            installed_by_entry[plugin_data.entry] = plugin_data  -- Also store with .koplugin suffix
        end
    end

    for _, repo_config in ipairs(plugin_repos or {}) do
        local owner = repo_config.owner
        local repo = repo_config.repo

        -- Skip if installed plugin matches this repository
        local found_installed = false
        
        -- First, try direct entry match (fastest)
        local repo_base = repo:gsub("%.koplugin$", "")
        if installed_by_entry[repo] or installed_by_entry[repo_base] then
            found_installed = true
        else
            -- Fallback to fuzzy matching by plugin_name
            for plugin_name, plugin_data in pairs(installed_map) do
                if PluginManager.matchPluginToRepo(plugin_name, repo, plugin_data) then
                    found_installed = true
                    break
                end
            end
        ...
        if not content or code ~= 200 then
            logger.warn("UpdatesManager: Failed to get latest release for install candidate:", api_url, code)
            goto continue
        end
        ...
        local zip_asset, asset_pattern = PluginManager.findZipAsset(release_data, repo_config)
        if not zip_asset then
            local pattern_info = asset_pattern and (" (pattern: " .. asset_pattern .. ")") or ""
            logger.warn("UpdatesManager: No matching asset found for install candidate:", repo .. pattern_info)
            goto continue
        end

        -- Extract version from tag_name (remove 'v' prefix if present)
        local release_version = (release_data.tag_name or ""):gsub("^v", "")

        -- Fake "installed plugin" so UI can reuse existing rendering / details / changelog
        local repo_base = (repo or ""):gsub("%.koplugin$", "")
        local pseudo_installed = {
            name = repo_base ~= "" and repo_base or (repo or "unknown"),
            fullname = repo_config.description or repo_base or repo or "unknown",
        ...
        table.insert(candidates, {
            installed_plugin = pseudo_installed,
